...
# convex hulls
convex_hulls = gpd.read_file("data/convex_hulls.gpkg")

# bldgs to replace in convex hulls
extracted = gpd.read_file("data/interesected.gpkg")

# bldgs that replace those in convex hulls
orig_bldg = gpd.read_file("data/bldg_polys.gpkg")

# make new dataframe from orig_bldg with ids frome extracted
relevant_fid2 = extracted.iloc[:,5].values
orig_bldg_geom = orig_bldg.iloc[:, 4:5]
...
# replace geometry
my_gdf = []
for idx, gdfrow in convex_hulls.iterrows():
...
relevant_gdf.to_file("data/rounded_bldgs.gpkg", driver="GPKG")